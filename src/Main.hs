    ...
    ; (ampersandOk, prototypeOk, buildTestResults) <-
        ...
        then
         do { putStrLn "Performing svn update for Ampersand"
            ...
            ; t2 <- reportTestResult opts $ testInstall "Ampersand" ["-f-executable","--ghc-option=-fprof-auto"] "the Ampersand library"
            ; t1 <- reportTestResult opts $ testInstall "Ampersand" ["--ghc-option=-fprof-auto"]                 "the Ampersand executable (and library)" -- cannot build exec without lib because of in-place dependency
