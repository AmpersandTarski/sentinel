    ...
    ; (ampersandOk, prototypeOk, buildTestResults) <-
        ...
        then
         do { putStrLn "Performing svn update for Ampersand"
            ...
            ; t2 <- reportTestResult opts $ testInstall "Ampersand" ["-f-executable","-p","--enable-executable-profiling","--ghc-option=-fprof-auto","--ghc-option=-prof"] "the Ampersand library"
            ; t1 <- reportTestResult opts $ testInstall "Ampersand" ["-p","--enable-executable-profiling","--ghc-option=-fprof-auto","--ghc-option=-prof"]                 "the Ampersand executable (and library)" -- cannot build exec without lib because of in-place dependency
            ; putStrLn "Cleaning Prototype"
            ; cabalClean "Prototype" []
            ; t3 <- reportTestResult opts $ testInstall "Prototype" ["-p","--enable-executable-profiling","--ghc-option=-fprof-auto","--ghc-option=-prof"] "the prototype generator"
